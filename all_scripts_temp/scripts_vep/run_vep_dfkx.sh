...
#SBATCH -t 99:00:00
date
#vep -i /home/huum/projs/regMotifs/mutations_cohorts_output_mut/mutations_simulations_files_103sets.out2_paj_un_pab -o /home/huum/projs/regMotifs/mutations_cohorts_output_mut/mutations_simulations_files_103sets.out2_paj_un_pab.vep.txt2 --dir_cache="/home/huum/tools/vep" --cache --port 3337 --pick_allele --offline

cat /home/huum/projs/regMotifs/mutations_simulations_files_103sets/simulation_dkfz_annotated.bed9 | awk 'BEGIN {FS=OFS="\t"}{if($4=="-") print $1 "\t" ($2+1) "\t" $2 "\t" $4 "/" $5; else if($5=="-") print $1 "\t" $2 "\t" ($2+(length($4))-1) "\t" $4 "/" $5; else print $1 "\t" $2 "\t" $3 "\t" $4 "/" $5}'  | sort -u -T /home/huum/projs/regMotifs/ > /home/huum/projs/regMotifs/mutations_cohorts_output_mut/simulation_dkfz_annotated.bed9.out2




vep -i /home/huum/projs/regMotifs/mutations_cohorts_output_mut/simulation_dkfz_annotated.bed9.out2 -o /home/huum/projs/regMotifs/mutations_cohorts_output_mut/simulation_dkfz_annotated.bed9.out2.vep.txt2 --dir_cache="/home/huum/tools/vep" --cache --port 3337 --pick_allele --offline

cat /home/huum/projs/regMotifs/mutations_simulations_files_103sets/simulation_dkfz_annotated.bed9 | awk 'BEGIN {FS=OFS="\t"}{if($4=="-") print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" ($2+1) "\t" $2 "\t" $4 "/" $5 "\t" $1 "_" ($2+1) "_" $4 "/" $5; else if($5=="-") print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" $2 "\t" ($2+(length($4))-1) "\t" $4 "/" $5 "\t" $1 "_" ($2) "_" $4 "/" $5; else print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" $2 "\t" $3 "\t" $4 "/" $5 "\t" $1 "_" ($2) "_" $4 "/" $5 }'  | sort -u -T /home/huum/projs/regMotifs/  > /home/huum/projs/regMotifs/mutations_cohorts_output_mut/simulation_dkfz_annotated.bed9.out2_id
