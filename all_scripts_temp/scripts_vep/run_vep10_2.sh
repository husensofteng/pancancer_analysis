...
at /home/huum/projs/regMotifs/mutations_simulations_files_103sets/simulation_dkfz_annotated.bed9 | awk 'BEGIN {FS=OFS="\t"}{if($4=="-") print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" ($2+1) "\t" $2 "\t" $4 "/" $5 "\t" $1 "_" ($2+1) "_" $4 "/" $5 ; else if($5=="-") print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" $2 "\t" ($2+(length($4))-1) "\t" $4 "/" $5 "\t" $1 "_" ($2) "_" $4 "/" $5; else print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $1 "\t" $2 "\t" $3 "\t" $4 "/" $5 "\t" $1 "_" ($2) "_" $4 "/" $5 }'  | sort -u -T /home/huum/projs/regMotifs/  > /home/huum/projs/regMotifs/mutations_cohorts_output_mut/simulation_dkfz_annotated.bed9.out2_id
