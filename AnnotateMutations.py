    ...
    with open(muts_tracks_file, 'r') as mut_tracks_ifile, open(muts_tracks_ouput_file, 'w') as mut_tracks_ofile:
        ...
        while len(l)>cancer_type_index:
            ...
            for track in tracks:
                ...
                try:
                    if cellname in tissue_cell_assays[l[cancer_type_index]]:
                        if cellname not in cell_names_used:
                            cell_names_used.append(cellname)
                        try:
            ...
            for anno in sorted(annotations.keys()):
                if anno=='TFBinding':
                    tf_binding_peak = True
                    annotations[anno] = ';'.join(set(annotations[anno]))
                    continue
                if anno=="DNase-seq":
                    dnase_peak = True
                try:
                    annotations[anno] = str(sum(annotations[anno])*1.0/(len(annotations[anno])*1.0))
            ...
            for x in sorted(annotations.keys()):
                try:
                    if float(annotations[x])>0.0:
                        annotations_combined.append(x+":"+annotations[x])
                except ValueError:
                    annotations_combined.append(x+":"+annotations[x])
            '''
...
def get_annotated_muts(muts_input_file, tracks_dir, muts_out, filter_on_dnase1_or_tf_peak=False):
    ...
    tissue_cell_mappings_file='/datafiles/TissueCellMatches'
